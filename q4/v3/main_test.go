...
func Test_checkLine(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// }
			if got := checkLine(tt.args.k, stringSliceToDesk(tt.args.desk), tt.args.i+1, tt.args.j, tt.args.di, tt.args.dj); got != tt.want {
				t.Errorf("checkLine() = %v, want %v", got, tt.want)
			}
		})
	}
...
func stringSliceToDesk(strDesk []string) []row {
	n, m := len(strDesk), len(strDesk[0])
	byteDesk := make([]row, n+2)

	byteDesk[0] = make(row, m+1)
	for i := 1; i <= n; i++ {
		row := append(make([]byte, 0, m+1), strDesk[i-1]...)
		byteDesk[i] = append(row, 0)
	}
	byteDesk[n+1] = make([]byte, m+1)
